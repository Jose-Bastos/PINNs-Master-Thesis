...
class Logger:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        self.update_terminal(iteration, MSE, Reg, l1_norm,mse_test=MSE_test)
    ...
    def update_terminal(self, iteration, MSE, Reg, L1,mse_test):
        """Prints and updates progress of training cycle in command line."""
        sys.stdout.write(
            f"\r{iteration:>6}  MSE: {MSE.item():>8.2e}   MSE_TEST: {mse_test.item():>8.2e}   Reg: {torch.sum(Reg).item():>8.2e}  L1: {torch.sum(L1).item():>8.2e} "
        )
        sys.stdout.flush()
